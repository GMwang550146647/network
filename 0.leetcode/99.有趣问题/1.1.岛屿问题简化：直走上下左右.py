...
class Tu():
    ...
    def find_direction(self,pos):
        x,y=pos
        possible_next_pos=np.array([[x-1,y],[x,y-1],[x,y+1],[x+1,y]])
    ...
    def plot(self,allroads):
        fig=plt.figure()
        for i in range(len(allroads)):
            axi=fig.add_subplot(15,15,i+1)
            roadi=np.array(allroads[i])
            x=roadi[:,0]
            y=roadi[:,1]
            axi.plot(x,y,'r')
            plt.xlim((0,4))
